...
class 导出器:
    ...
    def __init__(self, 上下文):
        ...
        self.错误提示列表: list[str] = []  # 收集因类型/数据不合法被跳过的表提示
    ...
    # 组合根名称与扩展名，数据表与配置表均直接以导出标记作为文件名。
    ...
    def 导出(self, 路径: str, 冲突标记: set = None) -> list[dict]:
        ...
        for 工作表名称 in [名称 for 名称 in 数据.sheets if type(名称) is str]:
            ...
            try:
                ...
                # 该表存在非法类型或数据错误，仅跳过当前表，其余表继续导出
                print(异常)
                self.错误提示列表.append(str(异常))

        # 整文件没有任何表导出成功且存在表错误时按失败处理，否则正常保存其余表
        if self.错误提示列表 and not any(记录.对象 for 记录 in self.记录列表):
            raise ValueError("\n".join(self.错误提示列表))

        return self.保存全部()

    # 导出单个工作表内部实现：识别表类型、解析内容并登记导出记录。
    def 导出单个表(self, 数据, 工作表名称: str, 导出标记: str) -> None:
        工作表 = 数据.sheets[工作表名称]
        配置标题信息 = self.获取配置表标题信息(工作表)
        if not 配置标题信息:
            根名称 = self.获取根名称(导出标记, True)
            条目名称 = 导出标记
        else:
    ...
    def 导出条目工作表(self, 工作表) -> tuple[collections.OrderedDict, collections.OrderedDict]:
        ...
        # 表头字段名重复校验：仅对可导出列（第 2 行类型非空且非 none、第 1 行有名称）判断，
        # 同表内字段名不可重复，否则整表报错跳过。
        字段名位置 = {}
        for 列索引 in range(1, 列数):
            类型 = 导表工具集.获取单元格值(类型行[列索引]).strip()
            ...
            if 类型 and 名称 and 类型.lower() != "none":
                if 名称 in 字段名位置:
                    raise ValueError(
                        f"{os.path.basename(self.路径)}的{self.工作表名称}内"
                        f"{导表工具集.获取列字母(字段名位置[名称] + 1)}1和"
                        f"{导表工具集.获取列字母(列索引 + 1)}1存在相同字段名：“{名称}”"
                    )
                字段名位置[名称] = 列索引
        ...
        try:
            for 列索引 in range(1, 列数):  # 跳过第 0 列（ID/key），该列仅作为分块键
                类型 = 导表工具集.获取单元格值(类型行[列索引]).strip()
                名称 = 导表工具集.获取单元格值(名称行[列索引]).strip()
                是否导出 = 类型.lower() != "none"  # 第二行类型为 none 的列整列不导出
                标题信息列表.append((类型, 名称, 是否导出))

                if self.上下文.代码生成器 and 是否导出:
                    if 类型 and 名称:
                        self.构建表达式(模式对象, 类型, 名称, None, True)
    ...
    # 遍历导出记录执行保存，并返回各表的导出摘要记录（含导出文件路径）。
    # 无论是否启用代码生成器均构建该列表，供入口层汇总机器可读结果使用。
